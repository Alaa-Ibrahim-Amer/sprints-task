const request_rec = fetch("http://localhost:5000/api/products/getRecent");
const getRecentProducts = async () => {
  let response_rec = await request_rec;
  let data_rec = await response_rec.json();
  addRecentProducts(data_rec.data)
};

const addRecentProducts=(data)=>{
    data.forEach(element => {
        document.getElementById("recent-product").innerHTML += showRecentProduct(element);
    });
}
